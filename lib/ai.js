...
- ORDEN DEL CATÁLOGO: la lista YA VIENE ROTADA ENTRE CATEGORÍAS (anillo, aro,
  ...
  de la semana.
- ${includeEmails ? `${emailsPerWeek} emails` : "0 emails"} que la marca envía a su lista de clientas,
  coordinados con los posts (mismo producto o tema el mismo día o cercano) para reforzar el
  mensaje en los dos canales.
...
Reglas de emails (el objetivo de un email es que la clienta abra, haga clic y compre;
un correo bonito que nadie abre no sirve de nada):
- REGLA POR ENCIMA DE TODAS: el asunto y el texto tienen que ser SOBRE EL PRIMER
  producto de "productIndexes", y solo pueden afirmar lo que aparece en su
  entrada del catálogo de más arriba. Si el producto se llama "Piedra Azul", el
  correo NO puede hablar de violeta; si no dice de qué material es, no lo digas.
  Nada de medidas, piedras, colores, baños ni cuidados inventados, y nada de
  plazos, envíos, cambios ni descuentos. Un correo cuyo texto no calza con la
  foto llega a miles de clientas y termina en devoluciones.
- "subject": máximo 50 caracteres. Concreto y con un motivo para abrir HOY. Di qué hay
  dentro, no lo obvio. "Los aros de perla que se agotaron en marzo" sirve; "Novedades de
  la semana" o "Newsletter" no. Sin MAYÚSCULAS gritadas, sin "!!!", máximo un emoji y
  solo si aporta. Nada de prometer descuentos, envíos ni plazos que no te hayan dado.
- "previewText": 40-90 caracteres que CONTINÚEN el asunto, nunca lo repitan. Es la segunda
  línea que se lee en la bandeja de entrada y decide la apertura tanto como el asunto.
- "heading": la promesa del asunto, ya dentro del correo, en 3-7 palabras.
- "intro": 2-3 frases. Primero el beneficio para ella (con qué lo usa, para qué ocasión,
  qué problema le resuelve), después la pieza. Sin relleno de marca ni "nos complace".
- "productIndexes": elige entre 3 y 5 productos. El primero es el protagonista del texto
  y sale grande arriba; los demás acompañan en la grilla. Un correo con un solo producto
  da un solo lugar donde hacer clic, y es la razón más común de que una campaña no venda.
  Que los acompañantes combinen con el primero (misma ocasión o categorías que se llevan
  bien), no productos al azar.
- "ctaText": un verbo y qué va a encontrar, 2-4 palabras ("Ver los aros nuevos", "Comprar
  el collar"). Nunca "Click aquí" ni "Más información".
- "ctaUrl": el productUrl del producto protagonista, o la colección si el correo es de varias piezas.
- "closing": una frase corta que invite a responder o preguntar. Nada de despedidas largas.
...
/**
 * Escribe el texto de UN email para unos productos concretos.
 *
 * Existe por un problema real encontrado el 14-ago-2026: había campañas cuyo
 * texto no correspondía a la foto — un correo titulado "Color violeta para tu
 * día" cuyo producto era "Argollas Mini con Piedra AZUL". Un correo así llega a
 * mil clientas, y la que compra esperando violeta pide devolución.
 *
 * La diferencia con el plan semanal es que aquí los productos NO los elige la
 * IA: se le entregan ya decididos y su único trabajo es escribir sobre ESOS.
 * Así el texto no puede quedar hablando de otra cosa.
 *
 * @returns {Promise<{subject, previewText, heading, intro, closing, ctaText}>}
 */
export async function generateSingleEmail({ brand, products = [], fecha = "", instruction = "" }) {
  if (!products.length) throw new Error("No hay productos con los que escribir el correo.");

  const voice = brand.voice || {};
  const lang = voice.language || "es";
  const currency = voice.currency || "CLP";
  const tono = voice.tone || "cálido, cercano y aspiracional";

  const lista = products
    .map((p, i) => {
      const precio = p.price ? ` — precio: ${p.price} ${p.currency || currency}` : "";
      const desc = p.description ? ` — descripción de la tienda: ${p.description}` : "";
      return `${i}. ${p.title || "(sin título)"}${precio}${desc}`;
    })
    .join("\n");

  const system =
    `Eres una experta en email marketing para marcas de e-commerce de joyería y ` +
    `accesorios. Escribes en ${lang === "es" ? "español de Chile" : lang}, con un tono ${tono}. ` +
    `Tu público: ${voice.audience || "mujeres 25-45 que aman los accesorios"}. ` +
    `Devuelves SIEMPRE y ÚNICAMENTE JSON válido, sin texto adicional, sin markdown.`;

  const prompt = `Marca: "${brand.name}".${fecha ? ` El correo sale el ${fecha}.` : ""}

Estos son LOS productos que van en el correo. El de arriba (índice 0) es el
protagonista: su foto va grande y es de él de quien habla el texto. Los demás
aparecen abajo, en fichas con su foto y su precio, y NO hace falta nombrarlos:
${lista}

REGLA MÁS IMPORTANTE, POR ENCIMA DE CUALQUIER OTRA:
El texto tiene que ser SOBRE EL PRODUCTO 0, y solo puede afirmar lo que aparece
en su título o en la descripción de la tienda de más arriba. Está PROHIBIDO
inventar o suponer:
- colores o piedras que no estén nombrados (si dice "Piedra Azul", es azul; no
  puedes decir violeta, verde ni morado),
- materiales, baños o quilates que no estén escritos,
- medidas, tallas, largos o pesos,
- que sea hipoalergénico, que no se ponga negro, que aguante el agua, la ducha
  o el mar, o cuánto dura el baño,
- plazos de envío, despacho gratis, cambios, devoluciones, stock o descuentos.
Si un dato no está arriba, el correo se escribe sin él o se invita a preguntar
por mensaje. Un dato inventado termina en una devolución y en una clienta
enojada, y es peor que un correo aburrido.
${instruction ? `\nINSTRUCCIÓN DE LA USUARIA (manda sobre lo demás, salvo la regla anterior): "${instruction}"\n` : ""}
El objetivo del correo es que la clienta abra, haga clic y compre:
- "subject": máximo 50 caracteres. Concreto, con un motivo para abrir hoy, y
  fiel al producto 0. Sin MAYÚSCULAS gritadas, sin "!!!", máximo un emoji.
- "previewText": 40-90 caracteres que CONTINÚEN el asunto, sin repetirlo.
- "heading": la promesa del asunto ya dentro del correo, 3-7 palabras.
- "intro": 2-3 frases. Primero para qué le sirve a ella (con qué lo usa, para
  qué ocasión), después la pieza. Sin relleno de marca.
- "ctaText": 2-4 palabras, un verbo y qué encontrará ("Ver las argollas").
  Nunca "Click aquí" ni "Más información".
- "closing": una frase corta que invite a preguntar o responder.
...
Devuelve EXACTAMENTE este JSON, sin nada más:
{
  ...
  return {
    subject: s(p.subject, 120),
    previewText: s(p.previewText, 160),
    heading: s(p.heading, 120),
    intro: s(p.intro, 600),
    ctaText: s(p.ctaText, 40) || "Ver la colección",
    closing: s(p.closing, 300),
  };
}
