...
/**
 * Devuelve una generación al cupo. Se usa cuando el plan se cobró pero no
 * llegó a crearse (error de la IA, timeout): un intento fallido no debe
 * gastarle cupo al usuario.
 */
export async function refundGeneration(uid) {
  const ref = db.collection("users").doc(uid);
  const snap = await ref.get();
  if (!snap.exists) return;
  const key = monthKey();
  if (((snap.data().usage || {})[key] || 0) <= 0) return;
  await ref.set({ usage: { [key]: FieldValue.increment(-1) } }, { merge: true });
}
