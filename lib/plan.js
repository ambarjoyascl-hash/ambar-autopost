...
import { generateWeeklyPlan, generateSinglePost, generateSingleEmail } from "./ai.js";
...
/**
 * Reescribe el TEXTO de una campaña para que hable de los productos que
 * realmente lleva. Es el botón "Reescribir el texto" del panel.
 *
 * Nace de un problema concreto (14-ago-2026): campañas cuyo texto describía una
 * pieza distinta a la de la foto. Aquí los productos no se tocan; lo único que
 * cambia son las palabras, y se escriben a partir de esos productos, así que no
 * pueden volver a desviarse.
 *
 * @param {string} emailId
 * @param {{instruction?: string}} opts
 */
export async function rewriteEmail(emailId, { instruction = "" } = {}) {
  const ref = db.collection("emails").doc(emailId);
  const snap = await ref.get();
  if (!snap.exists) throw new Error("Campaña no encontrada.");
  const email = { id: snap.id, ...snap.data() };

  if (email.status === "sent") {
    throw new Error("Esa campaña ya se envió: el texto que recibieron tus clientas no se puede cambiar.");
  }
  if (email.status === "sending") {
    throw new Error("Esa campaña se está enviando en este momento. Espera a que termine.");
  }

  const productos = email.bloques?.products || [];
  if (!productos.length) {
    throw new Error(
      "Esta campaña no tiene productos guardados, así que no hay sobre qué escribir. Genera un plan nuevo."
    );
  }
  ...
  const tz = brand.voice?.timezone || DEFAULT_TZ;
  const cuando = epochToZoned(email.scheduledFor || Date.now(), tz);

  const texto = await generateSingleEmail({
    brand,
    products: productos,
    fecha: cuando.date,
    instruction: String(instruction || "").slice(0, 300),
  ...
  const bloques = {
    ...
    intro: texto.intro,
    closing: texto.closing,
    ctaText: texto.ctaText,
    ctaUrl: email.bloques.ctaUrl || productos[0]?.productUrl || brand.websiteUrl || "",
  };

  // El HTML se guarda también, aunque al abrir y al enviar se vuelva a dibujar:
  // así la lista del panel y cualquier vista antigua muestran ya el texto nuevo.
  const built = buildEmailHtml({
    brand,
    email: { subject: texto.subject, previewText: texto.previewText, ...bloques },
    products: productos,
  });

  const patch = {
    subject: texto.subject,
    previewText: texto.previewText,
    heading: texto.heading,
    ctaText: texto.ctaText,
    ctaUrl: bloques.ctaUrl,
    bloques,
    html: built.html,
    plainText: built.plainText,
    reescritoEl: Date.now(),
    updatedAt: Date.now(),
    // El texto acaba de escribirse a partir de estos productos, así que la
    // advertencia de "no corresponde a su producto" deja de aplicar.
    textoDesalineado: null,
  };
  await ref.set(patch, { merge: true });
