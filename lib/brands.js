...
//     language: "es"
//   },
//   email: {                         // envío de campañas por SES
//     enabled: false,                // interruptor: apagado no manda nada
//     from: "Ámbar Joyas <ventas@ambarjoyas.cl>",  // identidad verificada en SES
//     replyTo: "hola@ambarjoyas.cl"
//   },
//   brandKit: {                      // identidad visual de los emails
//     logoUrl, color, colorBg, colorText, font, headingFont,
//     instagramUrl, footerNote
//   },
//   createdAt, updatedAt
// }
...
export function redactBrand(brand) {
  ...
  return {
    ...rest,
    pinterest: {
      connected: !!(pin.accessToken && pin.boardId),
      username: pin.username || "",
      boardName: pin.boardName || "",
      tokenUpdatedAt: pin.tokenUpdatedAt || null,
    },
    instagram: {
      igUserId: ig.igUserId || "",
      pageId: ig.pageId || "",
      username: ig.username || "",
      fbUserName: ig.fbUserName || "",
...
export async function createBrand(input = {}, ownerUid = null) {
  ...
  const doc = {
    ...
    brandKit: sanitizeBrandKit(input.brandKit),
    createdAt: now,
    updatedAt: now,
  };
...
export async function updateBrand(id, patch = {}) {
  ...
  if (patch.instagram) {
    ...
    // Si cambiaron la cuenta a mano, el @usuario y el perfil de Facebook
    // guardados ya no corresponden (se rellenan solos al reconectar por OAuth).
    if (next.instagram.igUserId !== (current.instagram || {}).igUserId) {
      next.instagram.username = "";
      next.instagram.fbUserId = "";
      next.instagram.fbUserName = "";
    }
  }
  if (patch.shopify) {
  ...
  if (Array.isArray(patch.media)) {
    next.media = sanitizeMedia(patch.media);
  }
  if (patch.email) {
    next.email = { ...(current.email || {}), ...sanitizeEmail(patch.email) };
  }
  if (patch.brandKit) {
    next.brandKit = { ...(current.brandKit || {}), ...sanitizeBrandKit(patch.brandKit) };
  }
...
// Fotos/videos subidos por el usuario (biblioteca de la marca).
function sanitizeMedia(list = []) {
  return list
    .slice(0, 100)
    .map((m) => ({
      url: String(m.url || "").slice(0, 1200),
...
/** Ajustes de envío de campañas. `enabled` es el interruptor que decide si el
 *  cron manda de verdad: sale apagado y solo lo enciende la usuaria. */
function sanitizeEmail(email = {}) {
  const out = {};
  if (email.enabled != null) out.enabled = !!email.enabled;
  if (email.from != null) out.from = String(email.from).trim().slice(0, 200);
  if (email.replyTo != null) out.replyTo = String(email.replyTo).trim().slice(0, 200);
  if (email.testTo != null) out.testTo = String(email.testTo).trim().slice(0, 200);
  return out;
}

/** Un color solo se guarda si es un hexadecimal de verdad; cualquier otra cosa
 *  entraría cruda en el `style=` del email. "" sirve para volver al default. */
function hexColor(v) {
...
/** Nombre de tipografía: solo lo que puede ir dentro de un font-family. */
function fontName(v) {
  return String(v || "").replace(/[^A-Za-z0-9 ,'\-]/g, "").trim().slice(0, 120);
}

function sanitizeBrandKit(kit = {}) {
  const out = {};
  if (kit.logoUrl != null) {
    const u = String(kit.logoUrl).trim().slice(0, 1200);
    out.logoUrl = /^https?:\/\//.test(u) ? u : "";
  }
  if (kit.instagramUrl != null) {
    const u = String(kit.instagramUrl).trim().slice(0, 300);
    out.instagramUrl = /^https?:\/\//.test(u) ? u : "";
  }
  if (kit.color != null) out.color = hexColor(kit.color);
  if (kit.colorBg != null) out.colorBg = hexColor(kit.colorBg);
  if (kit.colorText != null) out.colorText = hexColor(kit.colorText);
  if (kit.font != null) out.font = fontName(kit.font);
  if (kit.headingFont != null) out.headingFont = fontName(kit.headingFont);
  if (kit.footerNote != null) out.footerNote = String(kit.footerNote).slice(0, 300);
  return out;
}
