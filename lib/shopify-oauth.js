...
// `read_products` saca las fotos y precios para los posts. Los de clientes son
// para las campañas de email: `read_customers` arma la audiencia (solo los que
// aceptaron recibir marketing) y `write_customers` registra las bajas en la
// propia tienda, que es donde vive el consentimiento — así se respetan también
// en los correos que mande Shopify por su cuenta.
//
// OJO: esto es solo lo que se PIDE al conectar. Lo que Shopify concede lo
// manda la versión publicada de la app en el Dev Dashboard: si esa versión no
// declara estos alcances, la tienda queda igual con read_products aunque acá
// se pidan los tres (así estaba al 6-ago-2026).
