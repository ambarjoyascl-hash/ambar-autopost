...
export function normalizeBase(websiteUrl) {
...
async function tryShopify(origin) {
  ...
  for (const p of json.products) {
    ...
    candidates.push({
      ...
      // Todas las fotos del producto: la [0] suele ser fondo blanco y las
      // siguientes de uso/lifestyle — permiten variar el estilo visual.
...
/**
 * Identidad estable de un producto: el camino de su URL, sin dominio.
 * Se compara por aquí lo ya publicado, para que cambiar el dominio de la tienda
 * (ambar-8632.myshopify.com → ambarjoyas.cl) no haga que todo el catálogo
 * parezca nuevo y se repitan productos ya usados.
 */
export function claveProducto(url) {
  const s = String(url || "").trim();
  if (!s) return "";
  try {
    return new URL(s).pathname.replace(/\/+$/, "").toLowerCase();
  } catch (_) {
