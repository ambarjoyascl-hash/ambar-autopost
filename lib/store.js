...
const PRESENCE = "catalog/presence.json";
...
// ── Presencia (cuánta gente está viendo el live) ──
export async function getPresence() {
  try {
    const { blobs } = await list({ prefix: PRESENCE });
    const b = blobs.find((x) => x.pathname === PRESENCE) || blobs[0];
    if (!b) return {};
    const r = await fetch(b.url, { cache: "no-store" });
    if (!r.ok) return {};
    const d = await r.json();
...
export async function savePresence(map) {
  await put(PRESENCE, JSON.stringify(map), {
    access: "public",
    addRandomSuffix: false,
    contentType: "application/json",
    cacheControlMaxAge: 0,
